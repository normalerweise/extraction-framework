 ...
 */
class CustomIntegerParser( context : { def language : Language } ,
                     ...
                     validRange : Double => Boolean = (i => true)) extends DataParser
{
    ...
    override def parse(node : Node) : Option[(Long,String)] =
    {
        for( text <- StringParser.parse(node);
             (convertedText, largeNumberSurfaceString) = parserUtils.convertLargeNumbersWithSurfaceStringInfo(text);
             (value, valueSurfaceString) <- parseValue(convertedText) )
        {
          val surfaceString = largeNumberSurfaceString
          return Some((value * multiplicationFactor).round, surfaceString)
        }

        None
    }

    private def parseValue(input : String) : Option[(Double, String)] =   // double is returned because
    {
        val numberStr = if(strict) input.trim else IntegerRegex.findFirstMatchIn(input.trim) match
        {
            case Some(s) =>
              s.subgroups(0).toString() // s is the WHOLE MATCH, while we want the matching subgroup
            case None =>
            {
