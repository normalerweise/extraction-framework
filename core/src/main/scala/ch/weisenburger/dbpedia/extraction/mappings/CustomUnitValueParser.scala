...
import org.dbpedia.extraction.dataparser.DurationParser
import org.dbpedia.extraction.dataparser.ParsingErrors

class CustomUnitValueParser( extractionContext : {
                        ...
                        multiplicationFactor : Double = 1.0) extends DataParser
{
    ...
    private val unitRegexLabels = CustomUnitValueParser.cleanRegex(inputDatatype match
    {
    ...
    override def parse(node : Node) : Option[(Double, UnitDatatype, String, String)] =
    {
        ...
        {
            val (text, valueSurfaceString) = parserUtils.convertLargeNumbersWithSurfaceStringInfo(parseResult)
            ...
            catchUnitValue(text, valueSurfaceString, errors) match
            {
                ...
                case None =>
                {
                  // Do some crazy stuff nobody did before :-)  
                   for( value <- catchValue(text);
                       ...
                       result <- generateOutput(value, Some(unit), valueSurfaceString, errors)) {
                     return Some(result)
                   } 
                  ...
                  //No unit value found
                    ...
                    {
                        for( value <- catchValue(text);
                             result <- generateOutput(value, None, valueSurfaceString, errors) )
                        {
                            return Some(result)
                        }
                    }
     ...
     */
    private def catchTemplates(node : Node, errors : Option[ParsingErrors]) : Option[(Double, UnitDatatype, String, String)] = {
      ...
      var originalValueString: Option[String] = None
      ...
      // TODO: {{height|ft=5|in=7+1/2}}
       else if (templateName == "INRConvert")
      {
        // mandatory value
        val rupee_value = templateNode.property("rupee_value").orElse(templateNode.property("1")).get.children.collect{case TextNode(text, _) => text}.head

        val currency_formatting = templateNode.property("currency_formatting").orElse(templateNode.property("2")).getOrElse(PropertyNode("badLuck", List.empty, 0)).children.collect{case TextNode(text, _) => text}.headOption.getOrElse("-")

        // see http://en.wikipedia.org/wiki/Template:INRConvert
        val dimension = currency_formatting match {
          case "k" => 3
          case "m" => 6
          case "b" => 9
          case "t" => 12
        ...
        generateOutput(value.get, unit, originalValueString.getOrElse(null), errors)
    }
    ...
    private def catchDuration(input : String) : Option[(Double, UnitDatatype, String, String)] =
    {
        durationParser.parseToSeconds(input, inputDatatype) match
        {
            case Some(result) => Some((result, extractionContext.ontology.datatypes("second").asInstanceOf[UnitDatatype], input, ""))
            case None => None
        }
    }
     ...
     */
    private def catchUnitValue(input : String, valueSurfaceString: String, errors : Option[ParsingErrors]) : Option[(Double, UnitDatatype, String, String)] =
    {
        ...
        if (language == "en" || language == "ja" || language == "zh")
        {
            input match
            {
                ...
                case UnitValueRegex1b(value, unit) => generateOutput(value, Some(unit), valueSurfaceString, errors)
                case UnitValueRegex1c(value, unit) => generateOutput(value, Some(unit), valueSurfaceString, errors)
                case PrefixUnitValueRegex1(unit, value) if catchPrefixedUnit => generateOutput(value, Some(unit),valueSurfaceString, errors)
                case _ => None
            }
        }
        ...
        else
        {
            input match
            {
                case UnitValueRegex2b(value, unit) => generateOutput(value, Some(unit), valueSurfaceString, errors)
                case UnitValueRegex2c(value, unit) => generateOutput(value, Some(unit),valueSurfaceString, errors)
                case PrefixUnitValueRegex2(unit, value) if catchPrefixedUnit => generateOutput(value, Some(unit), valueSurfaceString,errors)
                case _ => None
            }
        }
    }
     ...
     */
    private def generateOutput(valueString : String, unitString : Option[String] = None, errors : Option[ParsingErrors]) : Option[(Double, UnitDatatype, String, String)] = generateOutput(valueString, unitString, null, errors)

    private def generateOutput(valueString : String, unitString : Option[String], originalValueString: String, errors : Option[ParsingErrors]) : Option[(Double, UnitDatatype, String, String)] =
    {
        val doubleValue = 
        {
            try
        ...
        Some(doubleValue, valueUnit, Option(originalValueString).getOrElse(valueString), unitString.getOrElse("")) 
    }
}

private object CustomUnitValueParser
{
    /**
